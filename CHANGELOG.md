...
- Conversational Slack layer (additive, gated by `ALFRED_INTENT_ROUTER_ENABLED`). A new `converse` intent answers read-only questions, why-questions, summaries, and banter directly. Tier-1 answers from the model's prompt context; when it cannot, the listener escalates ONCE to a richer read-only tier-2 turn with a hard 60s budget, a small read-only turn budget, and a read-only toolset hint. Read-only is enforced for real: the Claude side runs under `--permission-mode plan` and the Codex side under a `read-only` sandbox, not just a tool allowlist. A `status_facet` hint (`fleet` / `runs` / `plans`) lets the model steer the read-only status handler before the deterministic keyword cues run. Covered by new tests in `tests/test_slack_intent.py` and `tests/test_slack_conversational.py`.
- Persona voice layer for Slack (`ALFRED_PERSONA`). Two built-in presets (`butler` default, `gilfoyle`) inject a small voice block into both classification tiers. The persona is style only: it is injected as trusted SYSTEM framing ABOVE the untrusted message sentinel, never inside it, and it NEVER changes the action vocabulary, the JSON contract, the confirmation gates, or any safety rule. `resolve_persona` / `persona_name` expose the resolution.
- Persisted multi-turn `ConversationContext`. The Slack listener's bounded follow-up context is now optionally persisted to `$ALFRED_HOME/state/slack-conversation-context.json` (loaded at startup, saved on each record) so a listener restart does not lose an in-flight thread. It remains never authority for a mutation: every mutating intent still surfaces the confirmation card.
- Honest token-usage reporting for subscription (no-API-key) operation. `ClaudeResult` now carries real `tokens_in` / `tokens_out` / `cache_creation_tokens` / `cache_read_tokens` parsed from the result envelope's `usage` block. `SpendState.record_result` persists per-day token counters plus an `engine_breakdown` rollup while keeping `cost_usd_today` for back-compat, and `metrics.agent_metric` sums the new per-day token keys. Under a subscription the dollar figure is list-price noise; the token counts are the honest local signal. The desktop usage panel and `server/usage.py` surface Codex quota (the `rate_limits` 5h/weekly windows and plan type) and dedupe subagent token replays by deriving each day's contribution from the cumulative session total. Covered by `tests/test_honest_usage_reporting.py`.
- Shared agent profile model (`agent_runner.metadata`): `agent_profile` / `agent_label` / `AgentProfile` / `AgentTheme` give every codename a public-safe default display name, role title, purpose, and theme, overridable via `ALFRED_<CODENAME>_DISPLAY_NAME` / `ROLE_TITLE` / `PURPOSE` / `THEME`. Slack posts, CLI status, and the server now render a `Display · Role` label instead of the bare codename.
- `docs/DESIGN_SPEC.md` and `docs/ARCHITECTURE_RFC.md`: the canonical native-client product design spec and the code-organization RFC (target package shape, the config-driven generic runner, the typed event envelope and fail-closed gates, and the refactoring roadmap). New `decisions/0002-surface-architecture-and-design-direction.md` (ADR 0002) records the one-runtime-four-doors surface decision and the client design direction.
...
- `alfred serve` is now a headless localhost JSON/SSE API. The `--no-browser` flag and the browser auto-open are gone (a stale auto-open never has a UI to point at). The native client (and its Vite browser dev mode) is the only UI; mutating POSTs still require the per-launch `X-Alfred-Token` header. The `_token` HTML-form synchronizer path is removed along with the server-rendered routes that used it.
- The desktop client is reorganized to a lifecycle information architecture: Home, Ask, Pipeline, Fleet, Lessons. `PipelineView` replaces the Kanban board with hold/done actions and a follow-up convert action; `LifecycleCard` carries the work-card grammar; the Ask composer is a unified conversational input (Enter sends, Shift-Enter newline). A glass and density pass and fleet-status role labels round out the surface. The client reads and mutates entirely through the headless `alfred serve` API.
...
- The server-rendered `alfred serve` dashboard: `lib/server/templates/*.html`, `lib/server/static/` (logo, bundled fonts, stylesheet), `lib/server/formatting.py`, and the HTML/`/planning` routes and HTML-form token path in `lib/server/views.py`. Per ADR 0002 there is no web UI; the native client is the only GUI and `alfred serve` is a headless JSON/SSE API. The matching dashboard/HTML-form tests were dropped from `tests/test_server.py`.
