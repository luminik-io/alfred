...
- Per-agent Claude and Codex model controls in Alfred Desktop. Each agent drawer shows the active source, saves provider aliases through the token-protected local API, and makes fleet or per-agent environment overrides visible instead of pretending the saved value is active.
...
set up and steer by talking to it, remembers what your codebase has already
taught it without running a database service, and spends fewer tokens getting
the same work done. This cycle makes role identity stable and themeable, gives
the team a zero-daemon memory that recalls past lessons by default, adds a set
of optional efficiency and scale "batteries" you turn on only if you want them,
and rewrites the public positioning and docs for a non-technical reader. The
signed desktop and Linux binaries for this version are produced by a separate
release step and are not part of this entry.
...
- Conversational, Ask-driven onboarding in the desktop client. Alfred can run first-run setup as a conversation: it asks what you need, proposes a fixed set of allowlisted setup actions, previews team names, and executes each write only after you approve it, reusing the same gates and handlers as the stepped setup form. See `docs/ONBOARDING.md`.
- A conversational theme builder. You can assemble your agent team by describing it to Alfred, which proposes role names, previews them across Desktop, Slack, onboarding, and runtime status, and saves them without touching the underlying role contracts. See `docs/IDENTITY_AND_THEMES.md` and `docs/THEME_SYSTEM.md`.
- The embedded SQLite hybrid memory store, now the zero-daemon default recall layer (`lib/memory/sqlite_hybrid.py`). It keeps every promoted lesson in one SQLite file under the state root and retrieves in clean tiers: an always-on lexical arm using the FTS5 full-text index (falling back to substring matching when a SQLite build lacks FTS5), and an opt-in dense arm that adds vector search when the optional `sqlite-vec` extension and a local embedder are present. When both arms run, their results are combined with Reciprocal Rank Fusion. A fresh install gets working recall with nothing running. See `docs/MEMORY_PROVIDERS.md`.
- Typed, linked, time-aware lessons and a repository-profile injector (memory Phase 2). Lessons now carry a type and links to related lessons and can be recalled with awareness of when they were learned, and Alfred can inject a short profile of the repository being worked on so recall is grounded in that codebase.
- Memory consolidation and persisted reuse (memory Phase 3). Alfred merges near-duplicate lessons by meaning, keeps a union of where each merged lesson came from, and evicts under memory pressure, so the store stays compact and the consolidated result is saved rather than recomputed each time.
- A Postgres and pgvector scale-tier memory provider (`lib/memory/pgvector_provider.py`), an opt-in for operators who want to move recall onto a shared Postgres with vector search. The default is unchanged and nothing is sent anywhere by turning it off.
- A coding-fleet memory A/B benchmark that measures repeated-mistake-rate: the same task suite, which re-tempts previously-learned mistakes, is run once with memory on and once with a true no-memory control, and solutions are judged by deterministic markers. The benchmark ships with a deterministic offline stub solver so it runs in CI without a model, and can be pointed at a real engine for a genuine comparison. It reports the harness, not a headline product metric. See `docs/BENCHMARKS.md`.
- A tool-output compactor at the Claude Code tool-I/O boundary (`lib/tool_compactor.py`). It compacts verbose tool output only after a command confirms success (stripping control codes, de-duplicating, budgeting head and tail, collapsing all-green test runs) and tees the full, uncompacted output through on failure so nothing needed for debugging is lost. See `docs/TOOL_COMPACTOR.md`.
- Skeleton and delta code reads. Large source files can be read as a structure-first skeleton with targeted follow-up reads instead of whole-file dumps, so the model sees the shape of a file and pulls in only the parts it needs. See `docs/SKELETON_READS.md`.
- Blast-radius briefings in the code map. Before a change, Alfred can summarize what a given file or symbol is connected to, so planning starts from an honest view of what the change touches.
- An optional headroom compression engine behind a configuration-driven selector. The built-in, standard-library compactor is the default and the fallback; the optional `headroom-ai` engine (Apache-2.0 upstream, not vendored) can be selected for stronger compression. When it is not installed, every headroom path is a clean no-op that routes back to the built-in engine, so it is never a hard dependency. See `docs/COMPRESSION.md`.
- A battery picker for optional extras. A shared manifest (`lib/batteries.py`), an `alfred batteries` command to list, enable, and disable them, and a picker step in the desktop onboarding let you turn on Redis Agent Memory, the headroom compression engine, the code-structure memory server, dense SQLite embeddings, and the Postgres and pgvector scale tier. Alfred runs fully with zero batteries enabled.
- Curated skill packs. Alfred ships a license-audited registry of Claude Code skills (`skills/packs.toml`) and an `alfred skills` command to list, install, and check them. Small, permissively licensed skills (code review, security, frontend, debugging, and Vercel's React best practices) are vendored into the repo with their licenses kept intact; larger tools (gstack QA, headroom token compression) are reference-installed from source on request. Vendored installs need no network and work in CI; reference-installs require an explicit `--yes`, and the pip-installed headroom is pinned to the exact license-audited release. The command works from a source checkout, a deployed runtime (`deploy.sh` copies the registry into `$ALFRED_HOME`), and an installed wheel (`alfred-os skills`). See `docs/SKILLS.md`.
- `alfred demo` runs the whole loop in one command, with only an authenticated `claude` CLI and no GitHub, Slack, or tokens. It copies a small bundled sample repo into a temporary directory, then runs a real plan, an operator approval gate you press Enter on, a build in an isolated worktree, an adversarial review that catches a real bug planted in the sample (verified with an actual reproduction), a fix, and a local ship with a pull-request-style summary. It is honest by construction: a missing CLI, a failed model call, an engine that changes nothing, a failing sample test suite, or a reviewer with no explicit verdict all stop the run instead of faking success, and the closing line reports the measured run time. See `docs/DEMO.md`.

### Changed

- Role-slug is now the canonical agent identity across the runtime. Runtime files, scheduler labels, GitHub labels, worktrees, and engine overrides key off stable roles (`architect`, `senior-dev`, `reviewer`, `test-engineer`); the Batman cast is the default display theme layered on top, and custom themes and names resolve across Desktop, Slack, onboarding, and runtime status without changing those role contracts.
- Slack and the desktop Ask surface are now real conversational agents. A plain question gets a direct, streamed answer instead of a saved plan with a "File issue" button, and the plan-and-issue flow opens only when you are describing work to build. Long Ask conversations no longer run out of room: past a configurable length, Alfred keeps the opening task and the most recent turns and replaces the middle with one compact, model-written summary, condenses and retries once if a turn still overflows, records every condensation, and leaves short chats untouched.
- Themed agent names are escaped for Slack markup, so a custom team name that contains Slack formatting characters renders as plain text instead of breaking the message.
- The README and landing hero were rewritten to plain-language enterprise positioning, and the docs were audited end to end for the same voice, with new guides for identity and themes, conversational onboarding, memory providers, the tool compactor, skeleton reads, compression engines, and the demo. Long-tail references were swept to point at role-slug runners and the current environment keys.
- Alfred Desktop uses the exact local runtime URL the operator chooses instead of silently retrying or rewriting stale localhost ports, and the setup console shows the current doctor command.
- Batman removes a fully fanned-out parent's `agent:large-feature` queue label, marks it `batman:fanout-complete`, and closes it after every child issue is filed, so duplicate fan-out is prevented without counting the planning parent as shipped work.
- Memory injection is now ranked, decayed, and delta-aware, so only the lessons relevant to the current task are surfaced, older lessons lose weight over time, and repeated context is not re-injected, all within a character budget.
- The web dashboard's route setup was split into per-domain API routers, and the Slack planning listener was broken into focused collaborators, making both easier to read and extend without changing behavior.

### Security

- Contained a path-traversal risk in the conversational repository-grounding path. Repository-to-local checkout mappings are now constrained so a crafted repository reference cannot resolve outside the intended checkout directory, and the static analysis finding for that path is cleared.

### Fixed

- Every engineering runner now has a consecutive-failure self-halt. When a runner fails a configured number of times in a row it stops itself instead of burning through work in a broken state, and the invariant is applied uniformly across all runners.
...
- `lib/labels.py`: added `PLAN_PENDING_APPROVAL` constant (`agent:plan-pending-approval`) plus a long-form `LABEL_AGENT_PLAN_PENDING_APPROVAL` alias. Required by `lib/slack_surface/approval.py` and `lib/batman.py`.
...
- **Slack threading + Block Kit + severity colour stripes.** New `lib/slack_surface/posting.py` with bot-token-aware `firing_thread_root` / `firing_thread_reply` / `firing_thread_close`. Attachment duplicate-render guard baked in from day one. Honours `BATMAN_APPROVAL_CHANNEL` for routing.
...
[Unreleased]: https://github.com/luminik-io/alfred/compare/v0.6.0...HEAD
[0.6.0]: https://github.com/luminik-io/alfred/compare/v0.5.3...v0.6.0
