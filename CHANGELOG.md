...
- `alfred demo` gives you the whole loop in one run, with only an authenticated `claude` CLI and no GitHub, Slack, or tokens. It copies a small bundled sample repo into a temp dir, then runs a real plan, an operator approval gate you press Enter on, a build in an isolated worktree, an adversarial review that catches a real bug planted in the sample (verified with an actual reproduction), a fix, and a local ship with a pull-request-style summary. It is honest by construction: a missing CLI, a failed model call, an engine that changes nothing, a failing sample test suite, or a reviewer with no explicit verdict all stop the run instead of faking success, and the closing line reports the measured run time.
