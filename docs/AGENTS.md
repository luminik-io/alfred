...
| `automerge` (`bin/automerge.py`) | Auto-merge | every 15 min | all `agent:authored` PRs | SHA-guarded squash-merges PRs with enough exact-head approvals, zero unresolved threads, clean merge state, and no failing checks. Never touches non-`agent:authored` PRs. |
...
./bin/alfred doctor --lifecycle # validate architect parser, Slack approval, Claude OAuth
```
