    ...
    def _hook(failure_text: str, kind: str, retry: Callable[[], bool]) -> bool:
        def _attempt(attempt_index: int, category: RecoveryCategory) -> bool:
            ...
            if result.subtype != "success":
                return False
            # The turn fixed (and may already have pushed) the branch; re-run the
            # push path to confirm every gate now passes and the push lands.
            return retry()
...
def _push_or_preserve(
    ...
    *,
    ...
    ok, failure_text, release_outcome, kind = _do_push()
    if ok:
        return True

    # Bounded auto-recovery before the preserve/HOLD fallback. The hook re-runs
    # this same push path via ``_do_push`` to confirm the fix actually landed.
    if recover is not None and recover(failure_text, kind, lambda: _do_push()[0]):
        return True
...
""")
        ...
        if not _push_or_preserve(
            ...
            recover=_make_push_recovery_hook(repo, issue_num, events.firing_id, wt, branch, events),
        ):
            spend.increment(failures_today=1, consecutive_failures=1)
            return 0
        commit_subject = run(
