...
def main() -> int:
    ...
    if not workflow_validation.ok:
        ...
        return 0
    push_remote, pr_head = push_remote_and_pr_head(wt, repo, branch)
    push = push_current_branch(wt, branch, remote=push_remote)
    if push.returncode != 0:
        msg = (
            f"[{AGENT.upper()}-PUSH-FAILED] preserved worktree at {wt}; "
            f"{short(push.stderr or push.stdout, 300)}"
        )
        print(msg)
        slack_post(msg, severity="warn")
        spend.increment(failures_today=1, consecutive_failures=1)
        events.emit("firing_complete", outcome="push-failed")
        return 0
    commit_subject = run(
...
""")
    ...
    if pr_url:
        ...
        # A clean firing clears the fail streak so the self-halt gate only
        # trips on a genuine run of consecutive failures.
        spend.set(consecutive_failures=0)
        msg = (
