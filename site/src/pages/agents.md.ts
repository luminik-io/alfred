...
export const GET: APIRoute = async ({ site }) => {
  ...
  const lines: string[] = [
    ...
    "## What Alfred is",
    "",
    "Alfred is an open-source coordination and supervision layer for local coding agents.",
    "It coordinates authenticated Claude Code, Codex, and OpenCode CLI sessions on a",
    "Mac or Linux host you choose: macOS via launchd, Linux via",
    "systemd --user. Each agent is a named role (`architect` turns approved multi-repo",
    ...
    "You configure goals, repositories, roles, and approval rules. Alfred keeps",
    "the workflow moving until it has a pull request, a review finding, or a",
    "decision for the configured control surface.",
    "",
    ...
    "",
    "A coding CLI session handles the current prompt. Alfred schedules selected",
    "repository work and wraps each firing in locks, preflight checks, limits, and",
    "isolated worktrees. It is built for engineering work that should keep",
    "moving between sessions: planned features, review follow-ups,",
    "tests, reviewer comments, dependency bumps, docs gaps, and multi-repo rollouts.",
    "A scheduler fires",
    "each agent at a configured cadence. Shared runner controls add locks,",
    "preflight checks, and bounded retry. Roles that change or review code use",
    ...
    "```",
    "",
    "After `alfred doctor` reports healthy, the configured fleet is visible:",
    ...
    "automerge only lands PRs that satisfy your merge policy. Robin handles",
    "triage, Damian turns specs into bundle queues, Huntress and Gordon load",
    "with the fleet and self-idle until their staging smoke or ECS settings",
    "exist, Proof Telemetry reports anonymous usage totals when enabled, and",
    "the memory, code-map, cleanup, briefing, recap, shipped-summary, and",
    "fleet-doctor jobs keep the loop inspectable.",
    ...
    "  opens a worktree, runs its configured coding engine, opens a PR, and flips the",
    ...
    "  This fan-out path is part of the public repository.",
    "",
    ...
    "",
    "- Does not upload repositories to an Alfred-hosted service. The selected",
    "  coding CLI still sends the context it uses to its model provider.",
    "- Does not ask for or proxy model API keys. Alfred invokes your authenticated",
    "  Claude Code, Codex, or OpenCode CLI. Billing comes from the selected provider.",
    "- Sends no repo names, code, prompts, titles, branches, or people in usage totals.",
    "  Usage totals are sent only when Alfred telemetry is enabled;",
