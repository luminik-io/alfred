---
title: Alfred
description: Alfred turns Claude Code and Codex into a small, named engineering team that remembers. Agents plan, build, review each other, and open pull requests, and a run that learns a durable lesson teaches it to the fleet.
template: splash
head:
  - tag: title
    content: "Alfred: an engineering team, built on Claude Code and Codex, that remembers"
hero:
  tagline: "Named agents plan, build, review each other, and open pull requests, and a run that learns a durable lesson teaches it to the fleet."
  image:
...
<LinkCard
  ...
  description="Turn rough requests or specs into scoped issue queues, Architect plans, clean worktrees, reviewable PRs, and visible Slack summaries."
  href="/guides/specs-driven-development/"
/>

<LinkCard
  title="Plan in Slack"
  description="Reply to Architect plans with plain language or structured scope commands before Alfred files child issues."
  href="/concepts/slack-native-planning/"
/>

<LinkCard
  title="Add a custom role"
  description="The Echo tutorial: pick, claim, invoke, act, release, report. The shape every bespoke role inherits."
...
installs the CLI. The design boundary is stable: one operator or small team,
...
tests, new roles with clear scope, or optional integrations that fail
