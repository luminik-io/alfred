---
title: Alfred
description: An autonomous engineering team that ships while you're away.
template: splash
head:
  - tag: title
    content: "Alfred: autonomous coding agents for lean engineering teams"
hero:
  tagline: "Claude Code, Codex, or OpenCode plan work, write code, test it, review changes, and open pull requests. Alfred keeps working without you at the keyboard. You approve risky actions and decide what merges."
  image:
...
Alfred coordinates authenticated Claude Code, Codex, and OpenCode CLIs as short-lived
...
Alfred does not replace a coding CLI or proxy model traffic. It adds the
workflow, memory, isolation, and decision controls needed to operate several
CLI sessions as one autonomous engineering team.
...
- Claude Code, Codex, and OpenCode engine routing with authentication and readiness checks.
...
- Local coding CLI authentication, not a hosted model gateway.
...
Read the [roadmap](/about/roadmap/) for active engine, evidence, memory, and
...
Codex, and OpenCode are validated execution engines. Any other detected CLI
remains unsupported until it passes the same authentication, permission,
