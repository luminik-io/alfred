---
title: Alfred
description: Turn GitHub issues into scheduled local agent runs. Claude Code and Codex-capable development agents, launched by launchd or systemd, on a machine you control.
template: splash
head:
  - tag: title
    content: "Alfred: the solo builder's agent OS"
hero:
  tagline: Run Claude Code and Codex agents against real repos from a machine you control.
  image:
...
Most agent frameworks (crewAI, MetaGPT, OpenHands, AutoGPT-style loops) assume one long-running Python process, in-memory state, and a human at a REPL. That can be useful for interactive experiments. It is not the shape Alfred uses for unattended repo work.
