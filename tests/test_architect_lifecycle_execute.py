...
def test_approval_timeout_returns_no_execute():
    ...
    from slack_surface.approval import APPROVAL_TIMEOUT
...
def test_config_from_env_flags_legacy_batman_keys_without_fallback():
    ...
    assert cfg.parent_repo == ""
    assert cfg.auto_execute == "0"
    assert cfg.slack_channel == ""
...
def test_config_from_env_ignores_legacy_keys_after_architect_keys_are_set():
    from architect_lifecycle import ArchitectLifecycleConfig

    cfg = ArchitectLifecycleConfig.from_env(
        {
            "BATMAN_PARENT_REPO": "old/specs",
